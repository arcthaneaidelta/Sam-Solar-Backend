...
const service = ({ logger, makeService }) => {
  ...
  svc.on("session:new", (session, path) => {
    ...
    let customerData = session.locals.queriedCustomerData || session.customerData?.customer || session.customerData || {};
      ...
      session
        ...
        .llm({
          ...
          llmOptions: {
            ...
            metadata: {
            inbound: session.customerData?.metadata?.inbound || '',
            outbound: session.customerData?.metadata?.outbound || ''
            },
            selectedTools: [
              {
                temporaryTool: {
