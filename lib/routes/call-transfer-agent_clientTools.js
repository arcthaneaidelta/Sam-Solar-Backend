...
const handleCall = async (session) => {
  // Automatically trigger customer data query for inbound calls
  if (session.direction === "inbound") {
    session.sendCommand("toolCall", {
...
const onToolCall = async (session, evt) => {
  ...
  // Handle query-customer-data tool
  if (name === "query-customer-data") {
    let customerData = {};
    try {
      // Get the prospect's phone number from the inbound call
      const phoneNumber = session.from || session.customerData?.customer?.phone;
      
      if (!phoneNumber) {
        throw new Error("No phone number available for query");
      }

      logger.info({ phoneNumber }, "Querying customer data from n8n webhook");

      // Make HTTP request to n8n webhook with 3s timeout
      const response = await fetchWithTimeout('https://n8n.iqsolars.com/webhook-test/querydetails', {
        method: 'POST',
        headers: {
      ...
      logger.info({ customerData }, "Received customer data from n8n webhook");
      session.sendToolOutput(tool_call_id, {
        type: "client_tool_result",
        invocation_id: tool_call_id,
        result: `Customer data retrieved: ${customerData.name || 'Unknown'}, ${customerData.address || 'Unknown address'}`,
        customerData: customerData
      });
    } catch (err) {
      logger.error({ err }, "Error querying customer data from n8n webhook");
      session.sendToolOutput(tool_call_id, {
        type: "client_tool_result",
        invocation_id: tool_call_id,
        error_message: "Failed to query customer data"
      });
      customerData = {}; // fallback
    }
    // After data is available (or failed), start the LLM session for inbound calls
    if (session.direction === "inbound") {
      session
        .llm({
          vendor: "ultravox",
          ...
          toolHook: "/toolCall",
          llmOptions: {
            systemPrompt: getSystemPromptIncoming(customerData),
            firstSpeakerSettings: {
              agent: {
                text: incomingGreetingString,
              },
            },
            model: "fixie-ai/ultravox",
  ...
  // Handle call-transfer tool (existing logic)
...
const onFinal = (session, evt) => {
  ...
  // Handle Ultravox completion reasons
...
const service = ({ logger, makeService }) => {
  ...
  svc.on("session:new", (session, path) => {
    ...
    if (!apiKey) {
      ...
      if (isOutbound) {
        // Outbound: start LLM session immediately
        session
          .answer()
          .llm({
            vendor: "ultravox",
            ...
            toolHook: "/toolCall",
            llmOptions: {
              systemPrompt: getSystemPromptOutgoing(customerData),
              firstSpeakerSettings: {
                agent: {
                  text: getOutgoingGreetingString(customerData),
                },
              },
              model: "fixie-ai/ultravox",
              recordingEnabled: true,
        ...
        // Inbound: trigger query-customer-data tool, do NOT start LLM yet
        session.answer();
        session.sendCommand("toolCall", {
          name: "query-customer-data",
          args: {}
