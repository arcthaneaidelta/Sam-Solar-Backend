...
const onToolCall = async (session, evt) => {
  ...
  if (name === "query-customer-data") {
    // Get the actual phone number for inbound calls
    const actualPhoneNumber = session.direction === "inbound" 
      ? session.from 
      : session.to;
    ...
    logger.info({ 
      ...
      originalArgs: args 
    }, "Processing query-customer-data with actual phone number");
    ...
    if (actualPhoneNumber) {
      try {
        const response = await fetchWithTimeout("https://n8n.iqsolars.com/webhook/querydetails", {
          ...
          body: JSON.stringify({ phoneNumber: actualPhoneNumber })
        });
        
        const data = {
          type: "client_tool_result",
          invocation_id: tool_call_id,
          result: "Customer data lookup completed"
        };
        session.sendToolOutput(tool_call_id, data);
        
        logger.info({ actualPhoneNumber }, "Successfully sent webhook with real phone number");
      } catch (err) {
        logger.error({ err, actualPhoneNumber }, "Error calling webhook");
        const data = {
          type: "client_tool_result",
          invocation_id: tool_call_id,
          error_message: "Failed to lookup customer data"
        };
        session.sendToolOutput(tool_call_id, data);
      }
    } else {
      logger.warn("No phone number found in session");
      const data = {
        type: "client_tool_result",
        invocation_id: tool_call_id,
