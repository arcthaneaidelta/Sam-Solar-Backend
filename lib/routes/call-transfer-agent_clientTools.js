...
<PaceAndEnergyMatching>
# CRITICAL: Mirror the customer's speaking pace and energy level throughout the conversation
# Analyze their speech patterns in the first 10-15 seconds and adapt accordingly:

FAST TALKERS (rapid, urgent speech):
→ Match their energy with slightly quicker responses
→ Use shorter sentences and faster transitions
→ Maintain enthusiasm and urgency in your tone
→ Example pace: "Totally get that — sounds like you're in the zone today — let me get right to it..."

SLOW TALKERS (deliberate, measured speech):
→ Slow down significantly, add longer pauses
→ Use more deliberate phrasing with extended pauses
→ Take 1.5-2 seconds between sentences instead of 1 second
→ Example pace: "I completely understand... [pause] ... let me walk through this... [pause] ... step by step..."

MODERATE PACE (conversational, relaxed):
→ Use your standard pacing rules (1 second pauses, half-second for commas)
→ Maintain natural, conversational rhythm
→ Example pace: "That makes sense... So let me ask you this..."

ENERGY MATCHING:
- HIGH ENERGY: Match their enthusiasm, use exclamation points in your mental delivery
- LOW ENERGY: Speak more gently, use calming tones, avoid rushing
- STRESSED/RUSHED: Acknowledge their pace but gradually slow down to calm them
- CONFUSED/UNCERTAIN: Speak slower and more clearly, use simpler language

# Monitor throughout the call and adjust as their pace changes
# This creates subconscious rapport and trust through NLP mirroring
</PaceAndEnergyMatching>
...
**CRITICAL PACING RULES - MANDATORY:**
- STOP and BREATHE for a FULL SECOND after every period (.)
- PAUSE for half a second after every comma (,)
- SLOW DOWN - you are speaking too fast
- Count "one Mississippi" after each sentence before continuing
- Match the pace of a calm, relaxed human conversation
- If you rush through sentences, you sound like a robot

**SPEAKING SPEED:**
- Speak 30% SLOWER than normal
- Take your time - there's no rush
- Breathe between thoughts
- Sound human and natural, not rushed
...
SCRIPT TO FOLLOW BELOW: (follow this script to convert the most sales utilizing your thorough knowledge in tonality, objection handling, and pitching with NEPQ and NLP knowledge
  ...
  Out of curiosity, do your power bills typically run over $150 a month on average?
   ...
   You do own the home at [INSERT ACTUAL ADDRESS FROM QUERY-CUSTOMER-DATA TOOL RESPONSE], right?
    [gather]
...
1. "I'm not interested." (ONLY if this is a genuine objection, not knee-jerk dismissal)
    ...
    2. "What company is this?" / "Who are you with?"
    "Sorry for not clearing that up... This isn't a company but actually a program that works alongside other green initiatives providing alternative and more affordable electricity for your home." [gather]
    "Have you heard anything about the new fixed-rate energy programs in your area?" [gather]

  3. "We're not looking into solar right now."
  ...
  5. "I already looked into this before."
  ...
  6. "I've heard bad things about solar."
  ...
  8. "I don't want a lien on my house."
  ...
  10. "We don't use that much electricity."
  ...
  11. "I'm too old / won't be in the home long enough."
  ...
  13. "I'm in an HOA and they don't allow it."
  ...
  14. "I need to talk to my spouse first."
  ...
  15. "My roof isn't good / needs work."
...
3. ONLY THEN: Greet with EXACTLY: "Hey, uh — ${customerData?.firstName || '[NAME]'}?" then WAIT for their response
4. After greeting, use [gather] to pause and listen for their reply
5. Use their first name "${customerData?.firstName || '[NAME]'}" throughout the conversation
6. Reference their specific address "${customerData?.address || '[ADDRESS]'}" when discussing the program
...
// Add this function to send call metadata to your server
const sendCallMetadata = async (session, callEndReason = 'completed') => {
  const { logger } = session.locals;
  
  const callMetadata = {
    callSid: session.call_sid,
    direction: session.direction, // 'inbound' or 'outbound'
    from: session.from,
    to: session.to,
    duration: session.duration || 0,
    callEndReason: callEndReason,
    timestamp: new Date().toISOString(),
    customerData: session.locals.queriedCustomerData || session.customerData?.customer || {},
  };

  try {
    await fetchWithTimeout('https://n8n.iqsolars.com/webhook/call-metadata', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(callMetadata)
    }, 5000);
    
    logger.info({ callMetadata }, "Call metadata sent successfully");
  } catch (error) {
    logger.error({ error, callMetadata }, "Failed to send call metadata");
  }
};
  ...
  if (name === "call-transfer") {
    ...
    try {
      const data = {
        type: "client_tool_result",
        invocation_id: tool_call_id,
        result: "Awesome! Let me connect you with our scheduling team right now. Just give me one second here...",
      };
...
const onFinal = async (session, evt) => {
  ...
  // Send call metadata regardless of completion reason
  await sendCallMetadata(session, evt.completion_reason);
  session.locals.metadataSent = true; // Mark as sent
...
const onClose = async (session) => {
  const { logger } = session.locals;
  logger.info('Call session closed');
  
  // Send metadata on call close as well (backup in case onFinal doesn't fire)
  if (!session.locals.metadataSent) {
    await sendCallMetadata(session, 'closed');
    session.locals.metadataSent = true;
  }
};

const onError = async (session, err) => {
  const { logger } = session.locals;
  logger.error({ err }, 'Call session error');
  
  // Send metadata on error
  await sendCallMetadata(session, 'error');
};
...
const service = ({ logger, makeService }) => {
  ...
  svc.on("session:new", (session, path) => {
    session.locals = {
      ...
      metadataSent: false, // Flag to prevent duplicate sends
      callStartTime: new Date(), // Track call start time
    };
    ...
    session.locals.logger.info({
      ...
      to: session.to,
      path: path
    }, `New ${session.direction} call initiated`);
      ...
      session
        ...
        .llm({
          ...
          llmOptions: {
            ...
            metadata: Object.keys(session.customerData || {}).reduce((acc, key) => { acc[key] = String(session.customerData[key]); return acc; }, {}),
            selectedTools: [
              {
                temporaryTool: {
