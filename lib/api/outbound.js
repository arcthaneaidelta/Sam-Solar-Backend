...
const VOXIMPLANT_CONFIG = {
  accountId: process.env.VOXIMPLANT_ACCOUNT_ID || "9740807",
  apiKey: process.env.VOXIMPLANT_API_KEY || "d28508dc-27e7-4b18-956f-f9742de50b5e",
  applicationId: process.env.VOXIMPLANT_APPLICATION_ID || "10969042",
  applicationName: process.env.VOXIMPLANT_APPLICATION_NAME || "ultravox.solariq.n2.voximplant.com",
  baseUrl: "https://api.voximplant.com/platform_api/",
  // You'll need to add your purchased phone numbers here
  outboundNumbers: process.env.VOXIMPLANT_OUTBOUND_NUMBERS ? 
    process.env.VOXIMPLANT_OUTBOUND_NUMBERS.split(",") : 
    [] // Add your purchased numbers here when available
};
...
async function voximplantRequest(endpoint, params = {}) {
  try {
    const response = await axios.post(VOXIMPLANT_CONFIG.baseUrl + endpoint, {
